...
import static samuraiSudoku.onThread.deneme3;
 ...
 * @author Emre
 */
public class sThread11 extends Thread {
    ...
    public sThread11(int[][] sudokuTahtasi, int row, int col) {
        ...
        synchronized (this) {
            try {
                ...
                if (newSolveMethod()) {
                    ...
                    this.wait();
                    int newCount2 = solveCounter(0, 0, sudokuTahtasi, 0);
                    count = newCount2;
    ...
    public boolean koyulabilirMi(int row2, int col2, int number) {
        ...
        if ((row >= 6 && row <= 8) && (col >= 6 && col <= 8)) {
            ...
            for (int i = 6; i <= q; i++) {
                if (deneme3[i][col] == number) {
                    return false;
                } else if (deneme3[row][i] == number) {
                    return false;
                }
